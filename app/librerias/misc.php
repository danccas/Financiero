...
function process_join_arrays_vold($base, $keys) {
  if(!array_keys_required($base, $keys)) {
    return false;
  }
...
function fecha($x = null, $h = null) {
  $formato = 'd/m/Y';
  return !empty($x) ? date($formato, strtotime($x)) . (is_null($h) ? '' : ' ' . hora($x)) : 'SIN FECHA';
}
...
function _404($ERROR = null) {
  global $MENU, $USUARIO, $PAGINA_TITULO, $SUB_MENU;
