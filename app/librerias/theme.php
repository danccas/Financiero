...
final Class Theme {
  ...
  public static function controller($file, $params = null) {
    ...
    Route::getInstance()->web['controlador']      = $file;
    Route::getInstance()->web['dir']              = dirname($file) . '/';
    if(!is_null($params)) {
      if(is_array($params)) {
        extract($params);
      } else {
  ...
  static function JS($a) {
    return Theme::group_options('extra-js', null, $a);
  }
  static function CSS($a) {
    return Theme::group_options('extra-css', null, $a);
  }
