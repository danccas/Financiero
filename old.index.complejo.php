...
$debitos = null; /*$db->get("
  ...
  SELECT
    ...
    (
      ...
      (
        ...
        WHERE MOV.cuenta_id = C.id AND MOV.eliminado IS NULL AND MOV.moneda_id = C.moneda_id
          ...
          AND ((DATE(MOV.fecha) <= '" . $fecha_actual . "' AND MOV.efectuado IS TRUE) OR (DATE(MOV.fecha) > '" . $fecha_actual . "'))
      )" : "") . "
    ) as cierre,
    (
      COALESCE((
    ...
    COALESCE((
      ...
      FROM financiero.movimiento
      WHERE monto > 0 AND cuenta_id = C.id AND moneda_id = C.moneda_id AND eliminado IS NULL AND to_char(fecha, 'YYYY-MM') = '" . $mes_visto . "' AND DATE(fecha) <= '" . $fecha_visto . "'
      " . (strtotime($fecha_actual) < strtotime($fecha_visto) ? " AND ((DATE(fecha) <= '" . $fecha_actual . "' AND efectuado IS TRUE) OR (DATE(fecha) > '" . $fecha_actual . "')) " : " AND efectuado IS TRUE ") . "
    ), 0) as flujo_ingreso,
    COALESCE((
      SELECT SUM(CASE WHEN bloqueado IS FALSE THEN monto ELSE (monto * -1) END)
      FROM financiero.movimiento
...
FROM (
SELECT
    string_agg(DISTINCT CI.nombre, ',') as nombre,
    CI.fecha,
    M.nombre as moneda,
...
FROM (
  SELECT
    C.id,
    C.tipo_id,
    (SELECT id FROM financiero.cierre WHERE usuario_id = C.usuario_id AND cuenta_id = C.id AND moneda_id = M.id AND fecha < '{$fecha_visto}' ORDER BY id DESC LIMIT 1) as cierre_id
  FROM financiero.moneda M
  JOIN financiero.cuenta C ON C.usuario_id = {$usuario_id} 
  ) x
	JOIN financiero.cierre CI ON CI.id = x.cierre_id
  JOIN financiero.moneda M ON M.id = CI.moneda_id
  WHERE cierre_id IS NOT NULL
	GROUP BY CI.fecha, M.nombre, x.tipo_id, CI.moneda_id, CI.nombre) y
  WHERE gasto <> 0 OR ingreso <> 0 OR contable <> 0
  ORDER BY nombre;");*/
...
$mgrupos = null; /*$db->get("
  SELECT
    ...
    string_agg(DISTINCT U.usuario, ',') as usuarios,
    SUM((
      (
        SELECT
          COALESCE(SUM(CI.disponible), 0)
        FROM financiero.cierre CI
        JOIN financiero.cuenta C ON C.id = CI.cuenta_id AND C.tipo_id = 1
        WHERE CI.usuario_id = GU.usuario_id AND to_char(CI.fecha, 'YYYY-MM') = '" . $mes_cierre . "' AND CI.moneda_id = M.id
      )
      +
      (
        SELECT
          COALESCE(SUM(CASE WHEN MOV.bloqueado IS FALSE THEN MOV.monto ELSE (MOV.monto * -1) END), 0)
        FROM financiero.cuenta C
        JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.efectuado IS TRUE AND MOV.eliminado IS NULL
        WHERE C.usuario_id = GU.usuario_id AND MOV.moneda_id = M.id AND C.tipo_id = 1 AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "' AND DATE(MOV.fecha) <= '{$fecha_visto}'
          AND (C.eliminado IS NULL OR DATE(C.eliminado) > '" . $fecha_visto . "')
      )
    )) as disponible,
    SUM((
      SELECT
        SUM((SELECT COALESCE(contable, 0) FROM financiero.cierre WHERE usuario_id = C.usuario_id AND cuenta_id = C.id AND moneda_id = M2.id AND fecha < '{$fecha_visto}' ORDER BY id DESC LIMIT 1))
      FROM financiero.cuenta C
        JOIN financiero.moneda M2 ON 1 = 1
    ...
    SUM((
      ...
      FROM financiero.cuenta C
      JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.efectuado IS TRUE AND MOV.eliminado IS NULL
      WHERE C.usuario_id = GU.usuario_id AND MOV.moneda_id = M.id AND C.tipo_id = 2 AND DATE(MOV.fecha) <= '{$fecha_visto}' AND DATE(MOV.fecha) >= (SELECT DATE(fecha) FROM financiero.cierre WHERE cuenta_id = C.id AND moneda_id = M.id ORDER BY id DESC LIMIT 1)
        AND (C.eliminado IS NULL OR DATE(C.eliminado) > '" . $fecha_visto . "')
    ...
    SUM((
      SELECT
        COALESCE(SUM(CASE WHEN MOV.bloqueado IS FALSE THEN MOV.monto ELSE (MOV.monto * -1) END), 0)
      FROM financiero.cuenta C
      JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.efectuado IS TRUE AND MOV.eliminado IS NULL AND MOV.monto > 0
      WHERE C.usuario_id = GU.usuario_id AND MOV.moneda_id = M.id AND C.tipo_id = 2 AND DATE(MOV.fecha) <= '{$fecha_visto}' AND DATE(MOV.fecha) >= (SELECT DATE(fecha) FROM financiero.cierre WHERE cuenta_id = C.id AND moneda_id = M.id ORDER BY id DESC LIMIT 1)
        AND (C.eliminado IS NULL OR DATE(C.eliminado) > '" . $fecha_visto . "')
    )) as credito_pago,
    SUM((
  ...
  FROM financiero.grupo_usuario GU2
    JOIN financiero.grupo G ON G.id = GU2.grupo_id
    JOIN financiero.grupo_usuario GU ON GU.grupo_id = G.id
    JOIN financiero.usuario U ON U.id = GU.usuario_id
    JOIN financiero.moneda M ON 1 = 1
...
$prestamos = null; /*$db->get("
  SELECT
    P.*,
    M.nombre as moneda,
    (P.monto * (100 + P.interes) / 100) as pagar,
    (SELECT SUM(monto) FROM financiero.movimiento WHERE prestamo_id = P.id AND eliminado IS NULL AND efectuado IS TRUE AND monto < 0 AND DATE(fecha) <= '{$fecha_visto}') as pagado,
    (SELECT SUM(monto) FROM financiero.movimiento WHERE prestamo_id = P.id AND eliminado IS NULL AND efectuado IS FALSE AND monto < 0 AND DATE(fecha) >= '{$fecha_visto}') as pendiente,
    (SELECT COUNT(monto) FROM financiero.movimiento WHERE prestamo_id = P.id AND eliminado IS NULL AND efectuado IS FALSE AND monto < 0 AND DATE(fecha) >= '{$fecha_visto}') as pendiente_cuotas,
    (SELECT fecha FROM financiero.movimiento WHERE prestamo_id = P.id AND eliminado IS NULL AND efectuado IS FALSE AND DATE(fecha) >= '{$fecha_visto}' ORDER BY fecha ASC LIMIT 1) as proxima_fecha,
    (SELECT monto FROM financiero.movimiento WHERE prestamo_id = P.id AND eliminado IS NULL AND efectuado IS FALSE AND DATE(fecha) >= '{$fecha_visto}' ORDER BY fecha ASC LIMIT 1) as proximo_pago
  FROM financiero.prestamo P
  JOIN financiero.moneda M ON M.id = P.moneda_id
...
$table->setData(function() use($db, $mes_visto, $usuario_id) {
  ...
  return $db->get("
    ...
    FROM financiero.movimiento MOV
      JOIN financiero.categoria C ON C.id = MOV.categoria_id
      JOIN financiero.moneda M ON M.id = MOV.moneda_id
    WHERE MOV.usuario_id = {$usuario_id} AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "' AND MOV.eliminado IS NULL AND MOV.efectuado IS TRUE AND MOV.contable IS TRUE AND MOV.categoria_id <> 29
    GROUP BY C.id, M.id
...
$table->setData(function() use($db, $mes_visto, $usuario_id) {
  ...
  SELECT
    ...
    AVG(monto) as monto
  FROM (SELECT
      to_char(MOV.fecha, '%w') as dia,
      M.nombre as moneda,
      SUM(CASE WHEN MOV.monto < 0 THEN MOV.monto ELSE 0 END) as monto
    FROM financiero.movimiento MOV
      JOIN financiero.moneda M ON M.id = MOV.moneda_id
    WHERE MOV.usuario_id = {$usuario_id} AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "' AND MOV.efectuado IS TRUE
      AND MOV.bloqueado IS FALSE AND MOV.contable IS TRUE AND MOV.eliminado IS NULL
      AND MOV.categoria_id IN (2,3,22,24)
    GROUP BY MOV.fecha, MOV.moneda_id, M.nombre)x
  GROUP BY dia, moneda
  ORDER BY dia ASC, moneda");
}, function($n) use($DIAS) {
  return array(
...
FROM (SELECT
  ...
  M.monto,
  COUNT(*) as cantidad
FROM financiero.movimiento M
WHERE M.eliminado IS NULL AND M.efectuado IS TRUE AND ((DAYOFMONTH(M.fecha) = DAYOFMONTH(NOW()) AND M.categoria_id NOT IN (15)) OR (DAYOFWEEK(M.fecha) = DAYOFWEEK(NOW()) AND M.categoria_id NOT IN (15)))
GROUP BY M.descripcion, HOUR(M.fecha)
ORDER BY cantidad DESC
LIMIT 10)x
JOIN financiero.cuenta C ON C.id = x.cuenta_id AND C.eliminado IS NULL
JOIN financiero.tipo T ON T.id = C.tipo_id
LEFT JOIN financiero.banco B ON B.id = C.banco_id
JOIN financiero.moneda M ON M.id = C.moneda_id
JOIN financiero.moneda M2 ON M2.id = x.moneda_id
JOIN financiero.categoria CAT ON CAT.id = x.categoria_id
  ...
  SELECT
    ...
    (
      (
        ...
        FROM financiero.cierre CI
        JOIN financiero.cuenta C ON C.id = CI.cuenta_id AND C.tipo_id IN (1, 3)
        WHERE CI.usuario_id = {$usuario_id} AND to_char(CI.fecha, 'YYYY-MM') = '" . $mes_cierre . "' AND CI.moneda_id = M.id
      )
      +
      (
        SELECT
        ...
        FROM financiero.movimiento MOV
        JOIN financiero.cuenta C ON C.id = MOV.cuenta_id AND C.tipo_id IN (1, 3)
        WHERE MOV.usuario_id = {$usuario_id} AND MOV.moneda_id = M.id AND MOV.efectuado IS TRUE AND MOV.eliminado IS NULL AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "' AND DATE(MOV.fecha) <= '{$fecha_visto}'
      )
    ) as contable,
    (
      (
      ...
      (
        ...
        FROM financiero.cuenta C
        JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.efectuado IS TRUE AND MOV.eliminado IS NULL
        WHERE C.usuario_id = {$usuario_id} AND MOV.moneda_id = M.id AND C.tipo_id = 1 AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "' AND DATE(MOV.fecha) <= '{$fecha_visto}'
      )
    ) as disponible,
    (
      SELECT
        SUM((SELECT COALESCE(contable, 0) FROM financiero.cierre WHERE usuario_id = C.usuario_id AND cuenta_id = C.id AND moneda_id = M.id AND fecha < '{$fecha_visto}' ORDER BY fecha DESC, id DESC LIMIT 1))
      FROM financiero.cuenta C
    ...
    (
      ...
      WHERE C.usuario_id = {$usuario_id} AND C.tipo_id = 2
        ...
        AND DATE(MOV.fecha) <= '{$fecha_visto}'
        AND DATE(MOV.fecha) >= COALESCE((SELECT DATE(fecha) FROM financiero.cierre WHERE cuenta_id = C.id AND moneda_id = M.id AND fecha < '{$fecha_visto}' ORDER BY fecha DESC, id DESC LIMIT 1), ('{$fecha_cierre}'::date - INTERVAL '1' MONTH))
    ) as credito_consumido,
    (
      SELECT
        COALESCE(SUM(CASE WHEN MOV.bloqueado IS FALSE THEN MOV.monto ELSE (MOV.monto * -1) END), 0)
      FROM financiero.cuenta C
      JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.efectuado IS TRUE AND MOV.eliminado IS NULL AND MOV.monto > 0
      WHERE C.usuario_id = {$usuario_id} AND MOV.moneda_id = M.id AND C.tipo_id = 2 AND DATE(MOV.fecha) <= '{$fecha_visto}'::date
       AND DATE(MOV.fecha) >= COALESCE((SELECT DATE(fecha) FROM financiero.cierre WHERE cuenta_id = C.id AND moneda_id = M.id ORDER BY id DESC LIMIT 1), ('{$fecha_cierre}'::date - INTERVAL '1' MONTH))
    ) as credito_pago,
    (
      SELECT
       SUM(credito)
    ...
    (
      ...
      (
        SELECT
          COALESCE(SUM(CASE WHEN MOV.bloqueado IS FALSE THEN MOV.monto ELSE (MOV.monto * -1) END), 0)
        FROM financiero.cuenta C
        JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.eliminado IS NULL
        WHERE C.usuario_id = {$usuario_id} AND MOV.moneda_id = M.id AND C.tipo_id = 1 AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "' AND (C.eliminado IS NULL OR DATE(C.eliminado) > '" . $fecha_visto . "')
      )
    ) as proyectado
    FROM financiero.moneda M"); */
$mes_resumen = null; /*$db->get("
  SELECT
    M.id as moneda_id,
    M.nombre as moneda,
    SUM(CASE WHEN MOV.monto > 0 AND MOV.efectuado IS TRUE AND MOV.contable IS TRUE AND C.tipo_id = 1 AND DATE(MOV.fecha) <= '{$fecha_visto}' THEN MOV.monto ELSE 0 END) as ingreso,
    SUM(CASE WHEN MOV.monto < 0 AND MOV.efectuado IS TRUE AND MOV.contable IS TRUE AND DATE(MOV.fecha) <= '{$fecha_visto}' THEN MOV.monto ELSE 0 END) as gasto,
    SUM(CASE WHEN MOV.monto > 0 AND MOV.efectuado IS FALSE AND MOV.contable IS TRUE AND C.tipo_id = 1 THEN MOV.monto ELSE 0 END) as ingreso_pendiente,
    SUM(CASE WHEN MOV.monto < 0 AND MOV.efectuado IS FALSE AND MOV.contable IS TRUE THEN MOV.monto ELSE 0 END) as gasto_pendiente,
    SUM(CASE WHEN MOV.monto > 0 AND C.tipo_id = 1 AND MOV.contable IS TRUE THEN MOV.monto ELSE 0 END) as ingreso_proyectado,
    SUM(CASE WHEN MOV.monto < 0 AND MOV.contable IS TRUE THEN MOV.monto ELSE 0 END) as gasto_proyectado
  FROM financiero.moneda M
  JOIN financiero.movimiento MOV ON MOV.moneda_id = M.id
  JOIN financiero.cuenta C ON C.id = MOV.cuenta_id
  WHERE MOV.usuario_id = {$usuario_id} AND MOV.bloqueado IS FALSE AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "' AND MOV.eliminado IS NULL
  GROUP BY M.id, MOV.moneda_id
...
$estadistica_01 = null; /*$db->get("
SELECT
  mes,
  moneda_id,
  moneda,
  SUM(ingreso) as ingreso,
  string_agg(gasto::text, ',') as gasto,
  ...
  (
    (
      SELECT
        COALESCE(SUM(CI.disponible), 0)
      FROM financiero.cierre CI
      WHERE CI.usuario_id = x.usuario_id AND DATE(CI.fecha) = (SELECT MAX(fecha) FROM financiero.cierre WHERE usuario_id = x.usuario_id) AND CI.moneda_id = x.moneda_id
    ) + (
      SELECT
        COALESCE(SUM(CASE WHEN MOV.bloqueado IS FALSE THEN MOV.monto ELSE (MOV.monto * -1) END), 0)
      FROM financiero.cuenta C
      JOIN financiero.movimiento MOV ON MOV.contable IS TRUE AND MOV.cuenta_id = C.id AND MOV.eliminado IS NULL AND (MOV.efectuado IS TRUE OR (MOV.efectuado IS FALSE AND DATE(MOV.fecha) > (date_trunc('MONTH', now()) + INTERVAL '1 MONTH - 1 day')::date))
      WHERE C.usuario_id = x.usuario_id AND MOV.moneda_id = x.moneda_id AND C.tipo_id = 1 AND DATE(MOV.fecha) > (SELECT MAX(fecha) FROM financiero.cierre WHERE usuario_id = x.usuario_id)
      ...
      FROM financiero.cuenta C
      JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.efectuado IS TRUE AND MOV.eliminado IS NULL
      WHERE C.usuario_id = x.usuario_id AND MOV.moneda_id = x.moneda_id AND C.tipo_id = 1 AND to_char(MOV.fecha, 'YYYY-MM') = x.mes
    )
  ) as disponible
FROM (SELECT
    C.usuario_id,
    to_char(MOV.fecha, 'YYYY-MM') as mes,
    MOV.moneda_id,
    M.nombre as moneda,
    COALESCE(SUM(
      CASE WHEN MOV.contable IS TRUE AND MOV.monto > 0 AND MOV.eliminado IS NULL AND MOV.bloqueado IS FALSE AND (MOV.efectuado IS TRUE OR (MOV.efectuado IS FALSE AND DATE(MOV.fecha) > (date_trunc('MONTH', now()) + INTERVAL '1 MONTH - 1 day')::date)) THEN MOV.monto ELSE 0 END
    ), 0) as ingreso,
    COALESCE(SUM(
      CASE WHEN MOV.contable IS TRUE AND MOV.monto < 0 AND MOV.eliminado IS NULL AND MOV.bloqueado IS FALSE AND (MOV.efectuado IS TRUE OR (MOV.efectuado IS FALSE AND DATE(MOV.fecha) > (date_trunc('MONTH', now()) + INTERVAL '1 MONTH - 1 day')::date)) THEN MOV.monto ELSE 0 END
    ), 0) as gasto
  FROM financiero.cuenta C
  JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND DATE(MOV.fecha) > '{$fecha_cierre}'
  JOIN financiero.moneda M ON M.id = MOV.moneda_id
  WHERE C.usuario_id = {$usuario_id}
  GROUP BY to_char(MOV.fecha, 'YYYY-MM'), MOV.moneda_id, C.usuario_id, M.nombre) x
  )
  UNION
  (
    SELECT
    ...
    FROM financiero.cierre C
    JOIN financiero.cuenta CU ON CU.id = C.cuenta_id
    JOIN financiero.moneda M ON M.id = C.moneda_id
    WHERE C.usuario_id = {$usuario_id} AND CU.tipo_id = 1
    GROUP BY mes, C.moneda_id, M.nombre
    LIMIT 11
  )
  ) x
  GROUP BY mes, moneda_id, x.moneda
    ...
    $rp = $db->get("
      ...
      SELECT
        ...
        (
          ...
          FROM financiero.cierre CI
            JOIN financiero.cuenta C ON C.id = CI.cuenta_id AND C.tipo_id = 1
          WHERE CI.usuario_id = {$usuario_id} AND to_char(CI.fecha, 'YYYY-MM') = '" . $f . "' AND CI.moneda_id = M.id
        ) as disponible,
        (
          (
            SELECT
              COALESCE(SUM(CI.contable), 0)
            FROM financiero.cierre CI
              JOIN financiero.cuenta C ON C.id = CI.cuenta_id AND C.tipo_id = 2
            WHERE CI.usuario_id = {$usuario_id} AND CI.moneda_id = M.id
              AND DATE(CI.fecha) = (SELECT MAX(fecha) FROM financiero.cierre WHERE cuenta_id = C.id AND CI.fecha < '" . $last . "' LIMIT 1)
          ) + (
            SELECT
              COALESCE(SUM((SELECT SUM(monto) FROM financiero.movimiento WHERE prestamo_id = P.id AND eliminado IS NULL AND efectuado IS FALSE AND monto < 0)), 0)
            FROM financiero.prestamo P
            WHERE P.fecha_deposito <= '" . $last . "' AND P.moneda_id = M.id AND P.usuario_id = {$usuario_id}
          )
        ) as deuda
      FROM financiero.moneda M)x");
  } else {
    $rp = $db->get("
    SELECT
      *,
    ...
    FROM (
      SELECT
        ...
        (
          ...
          FROM financiero.cierre CI
            JOIN financiero.cuenta C ON C.id = CI.cuenta_id AND C.tipo_id = 1
          WHERE CI.usuario_id = {$usuario_id} AND to_char(CI.fecha, 'YYYY-MM') = '" . $mes_actual_cierre . "' AND CI.moneda_id = M.id
        ) as disponible1,
        (
          SELECT
            COALESCE(SUM(CASE WHEN MOV.bloqueado IS FALSE THEN MOV.monto ELSE (MOV.monto * -1) END), 0)
          FROM financiero.cuenta C
          JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.eliminado IS NULL
            AND DATE(MOV.fecha) > '" . $fecha_actual_cierre . "' AND to_char(MOV.fecha, 'MM')::integer <= " . ($k + 1) . " AND to_char(MOV.fecha, 'YYYY')::integer = " . date('Y') . "
            " . (date('m') == ($k + 1) ? 'AND MOV.efectuado IS TRUE' : "AND ((to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_actual . "' AND MOV.efectuado IS TRUE) OR (to_char(MOV.fecha, 'YYYY-MM') <> '" . $mes_actual . "'))") . "
          WHERE C.usuario_id = {$usuario_id} AND MOV.moneda_id = M.id AND C.tipo_id = 1
        ) as disponible2,
        '{$last}'::timestamp as deuda_fecha,
        (
          SELECT
            SUM((SELECT COALESCE(contable, 0) FROM financiero.cierre WHERE usuario_id = C.usuario_id AND cuenta_id = C.id AND moneda_id = C.moneda_id AND fecha < '{$last}'::timestamp ORDER BY fecha DESC, id DESC LIMIT 1))
          FROM financiero.cuenta C
          WHERE C.usuario_id = {$usuario_id} AND C.tipo_id = 2 AND (C.eliminado IS NULL OR DATE(C.eliminado) > '{$last}'::timestamp) AND C.moneda_id = M.id
        ) as deuda_cierre,
        (SELECT
          COALESCE(SUM(CASE WHEN MOV.bloqueado IS FALSE THEN MOV.monto ELSE (MOV.monto * -1) END), 0)
        FROM financiero.cuenta C
        JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.eliminado IS NULL
          AND DATE(MOV.fecha) > (SELECT fecha FROM financiero.cierre WHERE usuario_id = C.usuario_id AND cuenta_id = C.id AND moneda_id = C.moneda_id AND fecha < '{$last}'::timestamp ORDER BY fecha DESC, id DESC LIMIT 1) AND to_char(MOV.fecha, 'MM')::integer <= " . ($k + 1) . " AND to_char(MOV.fecha, 'YYYY')::integer = " . date('Y') . "
          " . (date('m') == ($k + 1) ? 'AND MOV.efectuado IS TRUE' : "AND ((to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_actual . "' AND MOV.efectuado IS TRUE) OR (to_char(MOV.fecha, 'YYYY-MM') <> '" . $mes_actual . "'))") . "
        ...
        (SELECT
          COALESCE(SUM((SELECT SUM(monto) FROM financiero.movimiento WHERE prestamo_id = P.id AND eliminado IS NULL AND efectuado IS FALSE AND monto < 0)), 0)
        FROM financiero.prestamo P
        WHERE P.fecha_deposito <= '" . $last . "'::timestamp AND P.moneda_id = M.id AND P.usuario_id = {$usuario_id} AND (P.fecha_cancelacion IS NULL OR P.fecha_cancelacion >= '" . $last . "'::timestamp)
        ) as deuda_prestamo
      FROM financiero.moneda M)x");
  }
  $reporte_anual[$m] = $rp;
  ...
  (
    (
      SELECT
        COALESCE(SUM(CI.disponible), 0)
      FROM financiero.cierre CI
      WHERE CI.usuario_id = {$usuario_id} AND to_char(CI.fecha, 'YYYY-MM') = '" . $mes_cierre . "' AND CI.moneda_id = x.moneda_id
    )
    +
    (
      SELECT
        COALESCE(SUM(CASE WHEN MOV.bloqueado IS FALSE THEN MOV.monto ELSE (MOV.monto * -1) END), 0)
      FROM financiero.cuenta C
      JOIN financiero.movimiento MOV ON MOV.cuenta_id = C.id AND MOV.eliminado IS NULL AND ((DATE(MOV.fecha) <= DATE(NOW()) AND MOV.efectuado IS TRUE) OR DATE(MOV.fecha) > DATE(NOW()))
      WHERE C.usuario_id = {$usuario_id} AND MOV.moneda_id = x.moneda_id AND C.tipo_id = 1 AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "' AND DATE(MOV.fecha) <= x.fecha
    )
  ) as disponible
FROM
  (SELECT
    ...
    M.id as moneda_id,
    M.nombre as moneda,
    COALESCE(SUM(CASE WHEN MOV.monto < 0 AND MOV.bloqueado IS FALSE AND ((DATE(MOV.fecha) <= DATE(NOW()) AND MOV.efectuado IS TRUE) OR DATE(MOV.fecha) > DATE(NOW())) THEN MOV.monto ELSE 0 END), 0) as gasto,
    COALESCE(SUM(CASE WHEN MOV.monto > 0 AND MOV.bloqueado IS FALSE AND ((DATE(MOV.fecha) <= DATE(NOW()) AND MOV.efectuado IS TRUE) OR DATE(MOV.fecha) > DATE(NOW())) THEN MOV.monto ELSE 0 END), 0) as ingreso
  FROM financiero.moneda M
  JOIN financiero.movimiento MOV ON MOV.usuario_id = {$usuario_id} AND MOV.moneda_id = M.id AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "' AND MOV.eliminado IS NULL
  GROUP BY DATE(MOV.fecha), MOV.moneda_id, M.id) x
...
$estadistica_03x = $db->get("
  ...
  FROM categoria C
  JOIN movimiento MOV ON MOV.categoria_id = C.id AND MOV.usuario_id = {$usuario_id} AND MOV.moneda_id = 1 AND to_char(MOV.fecha, 'YYYY-MM') = '" . $mes_visto . "'
    AND MOV.eliminado IS NULL AND MOV.bloqueado IS FALSE
  GROUP BY C.id
  ORDER BY cantidad DESC");
